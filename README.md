...
- **评测集**：`eval/golden_samples.jsonl` 15 条人工标注样本，`python eval/run_eval.py` 输出字段级准确率
- **单元测试**：`python tests/test_dedup.py`（去重、名录分类，不依赖网络）
- **独立 AI 评审**：每个开发阶段由独立上下文的 AI 评审员按 checklist 审查，报告见 `docs/reviews/`
- **运行留痕**：`crawl_runs` 记录每轮采集的新增/去重/失败数，`raw_items` 保留原始数据

## 项目过程

开发计划、阶段评审记录见 `docs/reviews/`。
